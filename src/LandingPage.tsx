  ...
  return <div className="landing">
    ...
    <main id="lp-main">
      ...
      <section className="lp-wrap lp-section lp-switch" aria-labelledby="lp-switch-title">
        <div className="lp-switch-copy"><p className="lp-eyebrow">Студія виросла. Час змінити інструменти.</p><h2 id="lp-switch-title">Досить керувати студією<br/><span>в Google-таблицях.</span></h2><p>Записи — у таблиці, завдання — у чаті, зарплата — на калькуляторі. Зберіть роботу команди в одному місці й приділіть більше уваги автомобілям та клієнтам.</p><a className="lp-secondary" href="/register">Навести лад у студії <ArrowRight size={17}/></a></div>
        <div className="lp-switch-points">
          <article><span className="lp-switch-icon"><Sheet size={21}/></span><div><p className="lp-switch-before">«У якій таблиці цей запис?»</p><h3>Кожне авто має своє замовлення</h3><p>Клієнт, послуги, майстер і статус роботи зібрані в одній картці.</p></div></article>
          <article><span className="lp-switch-icon"><MessageSquare size={21}/></span><div><p className="lp-switch-before">«Хто це робить і що вже готово?»</p><h3>Кожен знає свої завдання</h3><p>Власник бачить хід робіт, а кожен майстер — свої призначення та чекліст у кабінеті.</p></div></article>
          <article><span className="lp-switch-icon"><Calculator size={21}/></span><div><p className="lp-switch-before">«Скільки нарахувати за цю роботу?»</p><h3>Умови оплати вже збережені</h3><p>Відсоток або фіксована сума за послугу. Перевіряєте роботу й підтверджуєте нарахування.</p></div></article>
        </div>
      </section>
      <section className="lp-wrap lp-transfer" aria-labelledby="lp-transfer-title"><div><span className="lp-tag"><Sheet size={15}/> Імпорт клієнтів із CSV</span><h2 id="lp-transfer-title">Ваша база вже в таблиці?<br/>Перенесіть її за кілька кліків.</h2><p>Заповніть готовий шаблон або збережіть свою Google-таблицю чи Excel у CSV та завантажте в Detailflow. Імена, телефони й автомобілі перенесуться після вашого підтвердження — без ручного введення кожного клієнта.</p><div className="lp-transfer-actions"><a className="lp-secondary" href="/register">Почати зі своєї бази <ArrowRight size={17}/></a><a className="lp-secondary" href="/templates/detailflow-clients.csv" download="detailflow-clients.csv">Завантажити шаблон <ArrowDown size={17}/></a></div></div><ol><li><span>01</span><div><b>Завантажте CSV</b><p>Або вставте скопійовані рядки разом із заголовками.</p></div></li><li><span>02</span><div><b>Перевірте дані</b><p>Оберіть колонки імені, телефону та авто в попередньому перегляді.</p></div></li><li><span>03</span><div><b>Підтвердьте імпорт</b><p>Клієнти зі збігом телефону зіставляються з наявною базою.</p></div></li></ol></section>
      <section className="lp-wrap lp-section" id="features"><div className="lp-section-top"><div><p className="lp-eyebrow">Від клієнтської бази до керування студією</p><h2>Detailflow —<br/>більше, ніж CRM.</h2></div><p>Клієнти та записи — це початок. Далі — зміни майстрів, завдання по кожному авто, перевірка якості, нарахування та фінанси студії.</p></div>
      ...
      <section className="lp-wrap lp-section lp-stock" id="stock" aria-labelledby="lp-stock-title">
        ...
        ].map(item=><div className="lp-stock-row" role="row" key={item.name}><span role="cell"><b>{item.name}</b><small>{item.minimum}</small></span><span role="cell">{item.quantity}</span><span role="cell"><span className={item.low?'lp-stock-status is-low':'lp-stock-status'}>{item.low?'Докупити':'У запасі'}</span></span></div>)}</div><div className="lp-stock-hint"><Package size={18}/><span>1 позиція нижче мінімального запасу</span></div></div><p className="lp-demo-label">Ілюстрація складського обліку · демонстраційні дані</p></div>
      </section>
