...
export default function Analytics({bookings,expenses}:{bookings:Booking[];expenses:Expense[]}) {
  ...
  const days=[...new Set([...rows.map(b=>b.date),...costs.map(e=>e.date)])].sort()
  ...
  return <section className="content analytics-page"><div className="page-title"><div><p>Показники за вибраний період</p><h1>Аналітика студії</h1></div><div className="filter-actions">{(['today','week','month','all'] as const).map((value,i)=><button className="text-btn" key={value} onClick={()=>preset(value)}>{['Сьогодні','7 днів','Цей місяць','Увесь час'][i]}</button>)}</div>
    ...
    <p className="report-note">Вартість робіт рахується після знижок за датою запису. Завершена робота не є підтвердженням фактичного надходження грошей. Витрати враховано за датою витрати; фільтри майстра, послуги та статусу до них не застосовуються.</p>
    ...
    <div className="panel report-section"><h2>Динаміка за днями</h2><div className="report-scroll"><table><thead><tr><th>Дата</th><th>Записів</th><th>Завершені роботи</th><th>Планові роботи</th><th>Витрати студії</th></tr></thead><tbody>{days.map(day=><tr key={day}><td>{day}</td><td>{rows.filter(b=>b.date===day).length}</td><td>{money(completed.filter(b=>b.date===day).reduce((sum,b)=>sum+net(b),0))}</td><td>{money(planned.filter(b=>b.date===day).reduce((sum,b)=>sum+net(b),0))}</td><td>{money(costs.filter(e=>e.date===day).reduce((sum,e)=>sum+e.amount,0))}</td></tr>)}</tbody></table></div>{!days.length&&<p>Немає даних за вибраний період.</p>}</div>
  </section>
}
