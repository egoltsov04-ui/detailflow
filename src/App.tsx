...
export default function App() {
  ...
  useEffect(() => {
    if (!supabase || !tenantId) return
    void supabase.from('work_orders').select('id,client_id,staff_id,title,vehicle_label,service_summary,status,total,deposit,due_at,notes,created_at,status_changed_at,checklist,compensation_percent,compensation_fixed,submitted_for_review_at,review_note,staff_profiles(full_name),clients(full_name,vehicles(make,model,plate_number,notes)),appointments(vehicles(make,model,plate_number,notes))').eq('tenant_id',tenantId).order('created_at',{ascending:false}).then(({data,error})=>{
      if(error || !data) return
      setWorkOrders(data.map((item:any)=>({id:item.id,clientId:item.client_id,clientName:item.clients?.full_name || 'Клієнт',staffId:item.staff_id,staffName:item.staff_profiles?.full_name || '',title:item.title,vehicle:resolveOrderVehicle(item),serviceSummary:item.service_summary || '',status:item.status as WorkOrderStatus,total:Number(item.total),deposit:Number(item.deposit),dueAt:item.due_at || '',notes:item.notes || '',createdAt:item.created_at,statusChangedAt:item.status_changed_at || item.created_at,checklist:Array.isArray(item.checklist)?item.checklist:[],compensationPercent:item.compensation_percent===null?null:Number(item.compensation_percent),compensationFixed:item.compensation_fixed===null?null:Number(item.compensation_fixed),submittedForReviewAt:item.submitted_for_review_at || '',reviewNote:item.review_note || ''})))
    })
  },[tenantId,syncVersion])
  useEffect(()=>{
    if(!supabase || !tenantId)return
    void Promise.all([
      ...
      supabase.from('staff_earnings').select('id,staff_id,work_order_id,amount,status,accrued_at').eq('tenant_id',tenantId).order('accrued_at',{ascending:false}).limit(100)
  ...
  async function createBooking(item: Booking, goToCalendar = true) { setBookings(items => [...items, item]); setClients(items => { const existing = items.find(client => client.phone === item.phone); return existing ? items.map(client => client.id === existing.id ? {...client, visits:client.visits + 1, total:client.total + item.price, car:item.car || client.car} : client) : [...items, {id:item.id,name:item.client,phone:item.phone,car:item.car,visits:1,total:item.price}] }); if (!supabase || !tenantId) {setModal(false);if(goToCalendar)setPage('Календар');return} const staff=staffList.find(member=>member.name===item.tech); const service=await supabase.from('services').select('id').eq('tenant_id',tenantId).eq('name',item.service).maybeSingle(); let client=clients.find(member=>member.phone===item.phone); if(!client || typeof client.id !== 'string') { const created=await supabase.from('clients').insert({tenant_id:tenantId,full_name:item.client,phone:item.phone==='Не вказано'?null:item.phone}).select('id').single(); if(created.error || !created.data){setConnection('error');return} client={id:created.data.id,name:item.client,phone:item.phone,car:item.car,visits:1,total:item.price}; if(item.car && item.car !== 'Авто не вказано') { const vehicle=await supabase.from('vehicles').insert({tenant_id:tenantId,client_id:created.data.id,notes:item.car}); if(vehicle.error){setConnection('error');return} } } if(service.error || !service.data){setConnection('error');return} const startsAt=`${item.date}T${item.time}:00+03:00`, endsAt=new Date(new Date(startsAt).getTime()+item.durationMinutes*60000).toISOString(); const created=await supabase.from('appointments').insert({tenant_id:tenantId,client_id:client.id,staff_id:typeof staff?.id==='string'?staff.id:null,starts_at:startsAt,ends_at:endsAt,status:statusValue(item.status),source:'admin'}).select('id').single(); if(created.error || !created.data){setConnection('error');return} await supabase.from('appointment_services').insert({appointment_id:created.data.id,service_id:service.data.id,service_name:item.service,unit_price:item.price,duration_minutes:item.durationMinutes}); const linked=await supabase.from('work_orders').select('id,created_at,status_changed_at').eq('appointment_id',created.data.id).maybeSingle(); const generated=linked.data || (await supabase.from('work_orders').insert({tenant_id:tenantId,client_id:client.id,staff_id:typeof staff?.id==='string'?staff.id:null,title:item.service,vehicle_label:item.car || null,service_summary:item.service,status:staff?'assigned':'new',total:item.price,deposit:0,checklist:[{id:'service',title:item.service,done:false}],created_by:user?.id || null}).select('id,created_at,status_changed_at').single()).data; if(generated)await supabase.from('work_orders').update({vehicle_label:item.car||null}).eq('id',generated.id); if(generated)setWorkOrders(list=>[{id:generated.id,clientId:client!.id,clientName:item.client,staffId:typeof staff?.id==='string'?staff.id:null,staffName:staff?.name || '',title:item.service,vehicle:item.car || '',serviceSummary:item.service,status:staff?'assigned':'new',total:item.price,deposit:0,dueAt:'',notes:'',createdAt:generated.created_at,statusChangedAt:generated.status_changed_at || generated.created_at,checklist:[{id:'service',title:item.service,done:false}],compensationPercent:null,compensationFixed:null,submittedForReviewAt:'',reviewNote:''},...list]); setClients(items=>items.map(entry=>entry.id===item.id?{...entry,id:client.id}:entry)); setBookings(items=>items.map(entry=>entry.id===item.id?{...entry,id:created.data.id,clientId:client.id}:entry)); setModal(false);if(goToCalendar)setPage('Календар') }
  ...
  if(userRole==='master'&&!ownMaster)return <main className="access-portal">Завантаження профілю майстра…</main>
  ...
  return <div className="app-shell">
    ...
    <main><header><button className="hamburger" onClick={() => setMenuOpen(true)}><Menu/></button><div className="search"><Search size={18}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Пошук клієнтів, авто, записів…"/></div><div className="header-actions"><small title={connection === 'local' ? supabaseSetupMessage : undefined} style={{color:connection === 'connected' ? '#26754c' : '#777982'}}>{connection === 'connected' ? 'БД підключена' : connection === 'checking' ? 'Перевірка БД…' : connection === 'error' ? 'Потрібно увійти' : 'Локальний режим'}</small>{user ? <button className="locale" onClick={() => void supabase?.auth.signOut()}>Вийти</button> : <button className="locale" onClick={() => setAuthOpen(true)}>Увійти</button>}<span className="locale" title="Мова інтерфейсу — українська">UA</span><div className="profile-control"><button className="avatar" aria-label="Меню профілю" aria-expanded={profileMenu} onClick={()=>setProfileMenu(v=>!v)}>{user?.email?.slice(0,2).toUpperCase() || 'Г'}</button>{profileMenu && <div className="profile-options"><b>{user?.user_metadata?.full_name || 'Ваш профіль'}</b><small>{user?.email || 'Ви не увійшли'}</small><button className="text-btn" onClick={()=>{setPage('Налаштування');setProfileMenu(false)}}>Налаштування студії</button><button className="text-btn" onClick={()=>{setProfileMenu(false);if(user) void supabase?.auth.signOut();else setAuthOpen(true)}}>{user?'Вийти з акаунта':'Увійти'}</button></div>}</div></div></header>
      {page === 'Огляд' && <Dashboard name={user?.user_metadata?.full_name || "колего"} calendar={()=>setPage("Календар")} open={setPage} bookings={filtered.filter(b=>b.date===new Date(Date.now()-new Date().getTimezoneOffset()*60000).toISOString().slice(0,10))} staff={staffList} revenue={revenue} onCreate={() => setModal(true)} onStatus={(id,status)=>{void updateStatus(id,status).then(error=>{if(error)window.alert(error)})}}/>}
...
function Team({items,selected,select,bookings,earnings,shifts,onStatus,add,remove}:{items:Staff[],selected:string,select:(name:string)=>void,bookings:Booking[],earnings:{staffId:string;amount:number;accruedAt:string}[],shifts:{staffId:string;startedAt:string;endedAt:string|null}[],onStatus:(id:number|string,status:Status)=>void,add:(member:Staff,email:string)=>Promise<string>,remove:(id:number|string)=>void}) {
  ...
  const current=items.find(item=>item.name===selected)||items[0], month=new Date().toISOString().slice(0,7)
  const currentShift=current?shifts.find(shift=>String(shift.staffId)===String(current.id)&&!shift.endedAt):undefined
  const myBookings=bookings.filter(item=>item.tech===current?.name), earned=current?earnings.filter(item=>String(item.staffId)===String(current.id)&&item.accruedAt.slice(0,7)===month).reduce((sum,item)=>sum+item.amount,0):0
