...
export default function App() {
  ...
  return <div className="app-shell">
    <aside className={menuOpen ? 'sidebar open' : 'sidebar'}>
      ...
      <nav>{nav.filter(([label])=>label!=='Кабінет майстра'||staffList.some(item=>item.userId===user?.id)).map(([label, Icon]) => <button key={label} onClick={() => {setPage(label);setMenuOpen(false)}} className={page === label ? 'active' : ''}><Icon size={18}/>{label}</button>)}</nav>
